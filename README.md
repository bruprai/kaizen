...
- [x] Tags(which will be used to filter tasks by tags and differentiate between categories)
- [x] Keyboard shortcut **Alt + d** to mark the task done - Delete icon will be visible when you hover over the task
- [x] Visit previous day’s tasks
- [x] Plan tasks for the next day with #PFT tag which stands for plan for tomorrow
- [x] Local first
...
- [x] Task sort by days or categories
- [x] Upgrade UI
...
### Used type safe string enum in for status field in task model to make it easier to read, compare and maintain. (Mainly because I missed Dart Enum functionality which I use quite often there). - Pro here is that I don’t need to convert string to enum(like in dart) manually as TaskStatus is just a string, it works well with JSON data from an API or Database.
